...
int kp = 1; // Proportional constant for controller (tuning parameter)
  ...
  while(e){
    ...
    // Rotating counter-clockwise
    	...
    	if (-speed < 100) // motor does not react with too low inputs
      	speed = -100; 
      analogWrite(PWM1, 0);
      // the reason why we send "-speed" is that the sign of speed must be positive
      // what changes the direction instead is the order of signals 
      // (PWM2 => PWM1 in clockwise) VS
	  // (PWM1 => PWM2 in counter-clockwise)
