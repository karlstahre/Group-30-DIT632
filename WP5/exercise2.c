...
// The following code has been taken from the submission of WP4 - Exercise 1 - Group 30 (2022)
...
void setup()
{
  ...
  Timer1.initialize(3000000); // Initialize timer with interval specified by parameter
  Timer1.attachInterrupt(updateLeds); // Specify which function to call when interrupt happens
}
...
void updateLeds()
{
  // Store the current temperature in celsius and
